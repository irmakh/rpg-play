...
/**
 * Per-campaign SQLite store.
 *
 * This file used to open one localdb.db at module load. Under multi-tenancy
 * every campaign gets its own file, so the whole DB-bound API moved inside
 * openCampaignDb(dbFile) — the function bodies are unchanged, they just close
 * over a per-campaign `db` instead of a module-level one.
 *
 * Two campaigns therefore never share a connection, a prepared statement or a
 * transaction: cross-campaign reads are impossible by construction rather than
 * by remembering a WHERE clause.
 *
 * db/campaign-store.js caches one instance per campaign id; nothing else should
 * call openCampaignDb directly.
 */
...
export function openCampaignDb(dbFile) {
...
function createMedia(id, fields) {
...
function setPortrait(charId, mediaId) {
...
function createShopItem(id, fields) {
...
function createPurchaseLog(id, fields) {
...
function updateLootItem(id, fields) {
  if (!fields || Object.keys(fields).length === 0) return;
  const mapped = { ...fields };
...
function bulkUpdateLootTag(ids, tag) {
...
function bulkDeleteLootItems(ids) {
...
function createLootLog(id, fields) {
...
function listTreasuryItemsByMode(mode) {
...
function createTreasuryItem(id, fields) {
...
function createMonster(id, fields) {
...
function updateDrawing(id, fields) {
...
function importCharacters(characters, media) {
...
function importMonsters(monsters) {
...
function importTreasury(items, shopConfig, purchaseLogs, lootLogs) {
...
function importShop(shopConfig, shopItems, purchaseLogs) {
  importTreasury((shopItems || []).map(r => ({ ...shopRowToTreasury(r), id: r.id })), shopConfig, purchaseLogs, []);
}

function importLoot(lootItems, lootLogs) {
...
function exportAll() {
  ...
  // ── The campaign's data API ────────────────────────────────────────────────
  return {
    _db: db,
    _file: dbFile,
    close() { try { db.close(); } catch {} },
    listCharacters, getCharacter, createCharacter, updateCharacter, deleteCharacter, getLinkedTokens,
    listMedia, getMediaById, createMedia, setPortrait, deleteMedia, getShopConfig,
    ...
    listOrphanMonsterInitEntries, clearInitEntries, getInitState, setInitState, listDrawings, addDrawing,
